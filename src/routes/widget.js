...
(function () {
  ...
  function initWidget() {
    ...
    style.innerHTML = [
      ...
      '#bodrless-chat{position:fixed;bottom:90px;right:24px;width:390px;height:630px;background:var(--et-cream);z-index:999999;display:none;flex-direction:column;border-radius:20px;overflow:hidden;box-shadow:0 20px 60px rgba(30,42,94,0.18);font-family:Montserrat,sans-serif;}',
      ...
      '.et-suggestion{background:rgba(255,255,255,0.08);border:1px solid rgba(255,255,255,0.2);color:rgba(255,255,255,0.9);padding:5px 10px;border-radius:20px;font-size:11px;cursor:pointer;transition:all 0.2s;}',
      ...
      '.book{background:var(--et-red);color:white;border:none;padding:9px 18px;border-radius:20px;cursor:pointer;font-size:12px;font-weight:600;transition:all 0.2s;}',
      ...
      '#bodrless-input{flex:1;padding:10px 14px;border:1.5px solid var(--et-border);border-radius:20px;outline:none;font-size:12.5px;font-family:Montserrat,sans-serif;background:var(--et-cream);color:var(--et-navy);}',
    ...
    logoImg.onerror = function() { this.parentNode.innerHTML = "ET"; };
    ...
    function showNameForm(p, bookBtn) {
      ...
      confirmBtn.onclick = function() {
        ...
        fetch("https://bodrless-api-v2.onrender.com/api/trips/book", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            agencyId: "epic-travels",
            guestName: guestName,
    ...
    function addPackage(p, i) {
      ...
      var airline = p.transport && p.transport.airline ? p.transport.airline : "Flight";
      ...
      var hotelName = p.hotel && p.hotel.name ? p.hotel.name : "Hotel";
      ...
      if (hasTransfer) {
        var transferVehicle = p.transfers.vehicleType ? p.transfers.vehicleType : "Car";
        var transferPrice = p.transfers.price ? p.transfers.price : 0;
        pkgBody.appendChild(makeRow("Transfer:", p.transfers.provider, transferVehicle + " | $" + transferPrice));
      }
